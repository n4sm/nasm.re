...
tags: ["ctf", "nasm", "pwn", "linux", "file stream"]
category: "research"
draft: false
---
...
```
x
First is computed the amount of bytes that contains the read buffer (`have`). If we do not have the right amount of bytes within the read buffer we first try to copy data from the read buffer. Then we check we are mapping the whole file (and not only a part of it) with the use of `mmap_remap_check` (to avoid useless code I put it directly after the implementation of `_IO_file_xsgetn_mmap`), if it fails the file is unmapped and the vanilla file operations is used to read data from the file.
